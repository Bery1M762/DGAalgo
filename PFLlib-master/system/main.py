...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "MLR": # convex
        ...
        elif model_str == "CNN_v1":
            args.model = CNN_v1(num_channels=3, num_classes=args.num_classes).to(args.device)

        elif model_str == "CNN": # non-convex
            if "MNIST" in args.dataset or "DGA" in args.dataset:
            ...
            if "MNIST" in args.dataset or "DGA" in args.dataset:
                # MNIST 28x28 -> 最终特征图 4x4
                args.model = LeNet(feature_dim=50*4*4, bottleneck_dim=256, num_classes=args.num_classes).to(args.device)
            elif "Cifar10" in args.dataset:
                # Cifar10 32x32 -> 最终特征图 5x5
                # 【注意】你提供的 LeNet 代码中第一层写死了 nn.Conv2d(1, ...)，只接受单通道。
                # 如果要跑 Cifar10 (3通道)，请务必去 models.py 将第一层改为 nn.Conv2d(3, 20, kernel_size=5)
                args.model = LeNet(feature_dim=50*5*5, bottleneck_dim=256, num_classes=args.num_classes).to(args.device)
            else:
                # 其他数据集默认设置
        ...
        elif args.algorithm == "FedLKM":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedLKM(args, i)
...
if __name__ == "__main__":
    ...
    # FedLKM
    parser.add_argument('-lck', "--label_cluster_k", type=int, default=0,
                        help="Number of clusters for label-distribution K-Means (<=1 means auto)")
    # 指定聚类簇数 K：>1 时强制使用该 K，<=1 时自动选择 K。
    parser.add_argument('-lcmk', "--label_cluster_max_k", type=int, default=8,
                        help="Max cluster count when auto-selecting K for label-distribution K-Means")
    # 自动选 K 的上界：在 [2, max_k] 范围内用轮廓系数选择最优 K。
    parser.add_argument('-lkms', "--lkm_noise_sigma", type=float, default=0.10,
                        help="Noise sigma for FedLKM synthetic generation")
